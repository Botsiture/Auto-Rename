    ...
    try:
        ...
        if not admins:
            return await pro.edit(
                "<b>ᴜsᴇ ɪᴛ ʟɪᴋᴇ ᴛʜɪs:</b> <code>/add_admin 1234567890</code>\n<b>ᴏʀ:</b> <code>/add_admin 1234567890 </code>",
        ...
        for admin_id in admins:
            try:
                ...
                admin_list += f"<blockquote><b>❌ ɪɴᴠᴀʟɪᴅ ɪᴅ: <code>{admin_id}</code></b></blockquote>\n"
                continue
            ...
            try:
                ...
                admin_list += f"<b>• ɴᴀᴍᴇ: {user.mention}\n⚡ ɪᴅ: <code>{user_id}</code></b>\n\n"
            except Exception as e:
                admin_list += f"<blockquote><b>❌ ᴄᴀɴ'ᴛ ғᴇᴛᴄʜ ᴜsᴇʀ: <code>{user_id}</code></b></blockquote>\n"

        if successfully_added:
            await pro.edit(
                f"<b><u>✅ ᴀᴅᴍɪɴ(s) ᴀᴅᴅᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ</u></b>\n\n{admin_list}",
                reply_markup=reply_markup
            )
        else:
            await pro.edit(
                f"<b>❌ ɴᴏ ᴀᴅᴍɪɴs ᴡᴇʀᴇ ᴀᴅᴅᴇᴅ:</b>\n\n{admin_list.strip()}",
                reply_markup=reply_markup
            )
    except Exception as e:
        await pro.edit(f"<b>❌ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ:</b> <code>{str(e)}</code>")
    ...
    try:
        ...
        if len(admins) == 1 and admins[0].lower() == "all":
            if admin_ids:
                ...
                return await pro.edit(
                    "<b><blockquote>⚠️ ɴᴏ ᴀᴅᴍɪɴ iDs ᴛᴏ ʀᴇᴍᴏᴠᴇ.</blockquote></b>",
                    reply_markup=reply_markup
                )
        ...
        if admin_ids:
            ...
            for admin_id in admins:
                ...
                if id in admin_ids:
                    try:
                        user = await client.get_users(id)
                        passed += f"<b>• ɴᴀᴍᴇ: {user.mention}\n⚡ ɪᴅ: <code>{id}</code></b>\n\n"
                    except:
                        passed += f"<b>• ɪᴅ: <code>{id}</code></b>\n\n"
                    await rexbots.del_admin(id)
                else:
                    passed += f"<blockquote><b>⚠️ ɪᴅ <code>{id}</code> ɴᴏᴛ ғᴏᴜɴᴅ ɪɴ ᴀᴅᴍɪɴ ʟɪsᴛ.</b></blockquote>\n"

            await pro.edit(
                f"<b><u>✅ ʀᴇᴍᴏᴠᴇᴅ ᴀᴅᴍɪɴ ɪᴅ:</u></b>\n\n{passed}",
                reply_markup=reply_markup
            )
        else:
            await pro.edit(
                "<b><blockquote>⚠️ ɴᴏ ᴀᴅᴍɪɴ iDs ᴀᴠᴀɪʟᴀʙʟᴇ ᴛᴏ ᴅᴇʟᴇᴛᴇ.</blockquote></b>",
                reply_markup=reply_markup
            )
    except Exception as e:
        await pro.edit(f"<b>❌ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ:</b> <code>{str(e)}</code>")
...
async def get_admins(client: Client, message: Message):
    try:
        ...
        if not admin_ids:
            admin_list = "<b><blockquote>❌ ɴᴏ ᴀᴅᴍɪɴs ғᴏᴜɴᴅ.</blockquote></b>"
        else:
            admin_list = ""
            for idx, id in enumerate(admin_ids, 1):
                try:
                    user = await client.get_users(id)
                    admin_list += f"<b>{idx}. ɴᴀᴍᴇ: {user.mention}\n⚡ ɪᴅ: <code>{id}</code></b>\n\n"
                except:
                    admin_list += f"<b>{idx}. ɪᴅ: <code>{id}</code></b>\n\n"

        reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("ᴄʟᴏsᴇ", callback_data="close")]])
        await pro.edit(
            f"<b>⚡ ᴄᴜʀʀᴇɴᴛ ᴀᴅᴍɪɴ ʟɪsᴛ:</b>\n\n{admin_list}",
            reply_markup=reply_markup
        )
    except Exception as e:
        await pro.edit(f"<b>❌ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ:</b> <code>{str(e)}</code>")
    ...
    try:
        ...
        if len(command_parts) < 2:
            await message.reply_text(
                "<b>ᴜsᴇ ɪᴛ ʟɪᴋᴇ ᴛʜɪs:</b> <code>/ban &lt;ᴜsᴇʀ_ɪᴅ&gt; [ʀᴇᴀsᴏɴ]</code>"
            )
            return

        user_id_str = command_parts[1]
        reason = command_parts[2] if len(command_parts) > 2 else "ɴᴏ ʀᴇᴀsᴏɴ ᴘʀᴏᴠɪᴅᴇᴅ"
        ...
        await message.reply_text(
            f"<b>🚫 ᴜsᴇʀ ʙᴀɴɴᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ</b>\n\n"
            f"<b>👤 ᴜsᴇʀ: {user_mention}\n"
            f"⚡ ᴜsᴇʀ ID: <code>{user_id}</code>\n"
            f"📝 ʀᴇᴀsᴏɴ: {reason}\n"
        ...
        try:
            await bot.send_message(
                chat_id=user_id,
                text=f"<b>🚫 ʏᴏᴜ ʜᴀᴠᴇ ʙᴇᴇɴ ʙᴀɴɴᴇᴅ</b>\n\n"
                     f"<blockquote><b>ʀᴇᴀsᴏɴ: {reason}\n"
                     f"ᴅᴀᴛᴇ: {date.today().strftime('%d-%m-%Y')}</b></blockquote>",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ᴄᴏɴᴛᴀᴄᴛ ᴀᴅᴍɪɴ", url=ADMIN_URL)]])
            )
        except:
            pass
...
async def unban_user(bot, message):
    try:
        if len(message.text.split()) < 2:
            await message.reply_text(
                "<b>ᴜsᴇ ɪᴛ ʟɪᴋᴇ ᴛʜɪs:</b> <code>/unban &lt;ᴜsᴇʀ_ɪᴅ&gt;</code>"
            )
            return
        ...
        await message.reply_text(
            f"<b>✅ ᴜsᴇʀ ᴜɴʙᴀɴɴᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ</b>\n\n"
            f"<b>👤 ᴜsᴇʀ: {user_mention}\n"
        ...
        try:
            await bot.send_message(
                chat_id=user_id,
                text=f"<b>✅ ʏᴏᴜ ʜᴀᴠᴇ ʙᴇᴇɴ ᴜɴʙᴀɴɴᴇᴅ</b>\n\n"
                     f"<blockquote><b>ʏᴏᴜ ᴄᴀɴ ɴᴏᴡ ᴜsᴇ ᴛʜᴇ ʙᴏᴛ ᴀɢᴀɪɴ!\n"
...
async def banned_list(bot, message):
    try:
        ...
        async for user in cursor:
            count += 1
            uid = user['_id']
            reason = user.get('ban_status', {}).get('ban_reason', 'ɴᴏ ʀᴇᴀsᴏɴ')
            ...
            lines.append(
                ...
                f"⚡ ɪᴅ: <code>{uid}</code>\n"
                f"📝 ʀᴇᴀsᴏɴ: {reason}\n"
                f"📅 ғᴀᴛᴇ: {banned_date}</b>\n"
            )

        if not lines:
            await msg.edit(
                "<b><blockquote>✅ ɴᴏ ᴜsᴇʀ(s) ɪs ᴄᴜʀʀᴇɴᴛʟʏ ʙᴀɴɴᴇᴅ</blockquote></b>",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ᴄʟᴏsᴇ", callback_data="close")]])
            )
        else:
            banned_text = f"<b>🚫 ʙᴀɴɴᴇᴅ ᴜsᴇʀs ʟɪsᴛ</b>\n\n{''.join(lines[:50])}"
            if len(lines) > 50:
        ...
        await msg.edit(f"<b>❌ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ:</b> <code>{str(e)}</code>")
...
async def remove_premium(client, message):
    try:
        if len(message.command) == 2:
            ...
            if hasattr(rexbots, "remove_premium_access"):
                if await rexbots.remove_premium_access(user_id):
                    ...
                    await client.send_message(
                        chat_id=user_id,
                        text=f"<b>ʜᴇʏ {user.mention},\n\n<blockquote>Yᴏᴜʀ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss ʜᴀs ʙᴇᴇɴ ʀᴇᴍᴏᴠᴇᴅ. ᴛʜᴀɴᴋs ғᴏʀ ᴜsɪɴɢ ᴏᴜʀ sᴇʀᴠɪᴄᴇs. ᴜsᴇ /ᴘʟᴀɴ ᴛᴏ ᴄʜᴇᴄᴋ ᴏᴛʜᴇʀ ᴘʟᴀɴs...!!</blockquote></b>"
                    )
                else:
            ...
            await message.reply_text("ᴅᴜᴅᴇ ᴜsᴇ ɪᴛ ʟɪᴋᴇ ᴛʜɪs /remove_premium <ᴜsᴇʀ_ɪᴅ>")
    except Exception as e:
...
async def myplan(client, message):
    ...
    if data and data.get("expiry_time"):
        ...
        await message.reply_text(
            f"<b>ʜᴇʏ {user},\n\n<blockquote>ʏᴏᴜ ᴅᴏ ɴᴏᴛ ʜᴀᴠᴇ ᴀɴʏ ᴀᴄᴛɪᴠᴇ ᴘʀᴇᴍɪᴜᴍ ᴘʟᴀɴ, ɪғ ʏᴏᴜ ᴡᴀɴᴛ ᴛʜᴇɴ ᴄʟɪᴄᴋ ᴏɴ ᴛʜᴇ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ...!!</blockquote></b>",
...
async def get_premium(client, message):
    try:
        if len(message.command) == 2:
            ...
            await message.reply_text("ᴅᴜᴅᴇ ᴜsᴇ ɪᴛ ʟɪᴋᴇ ᴛʜɪs /premium_info <ᴜsᴇʀ_ɪᴅ>")
    except Exception as e:
        await message.reply_text(f"❌ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ: {str(e)}")
# ----------------------------------------
# 𝐌𝐀𝐃𝐄 𝐁𝐘 𝐀𝐁𝐇𝐈
    ...
    try:
        if len(message.command) == 4:
            ...
            if seconds > 0:
                ...
                await message.reply_text("ɪɴᴠᴀʟɪᴅ ᴛɪᴍᴇ ғᴏʀᴍᴀᴛ. ᴘʟᴇᴀsᴇ ᴜsᴇ '1 ᴅᴀʏ ғᴏʀ ᴅᴀʏs', '1 ʜᴏᴜʀ ғᴏʀ ʜᴏᴜʀs', ᴏʀ '1 ᴍɪɴ ғᴏʀ ᴍɪɴᴜᴛᴇs', ᴏʀ '1 ᴍᴏɴᴛʜ ғᴏʀ ᴍᴏɴᴛʜs' ᴏʀ '1 ʏᴇᴀʀ ғᴏʀ ʏᴇᴀʀ'.")
        else:
            await message.reply_text("ᴅᴜᴅᴇ ᴜsᴇ ɪᴛ ʟɪᴋᴇ ᴛʜɪs: `/add_premium <ᴜsᴇʀ_ɪᴅ> <ᴛɪᴍᴇ_ᴠᴀʟᴜᴇ> <ᴛɪᴍᴇ_ᴜɴɪᴛ>`.\n\nExample: `/add_premium 1234567890 30 Ɖαʏs`")
    except Exception as e:
        await message.reply_text(f"❌ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ: {str(e)}")
# ----------------------------------------
# 𝐌𝐀𝐃𝐄 𝐁𝐘 𝐀𝐁𝐇𝐈
...
async def premium_user(client, message):
    try:
        ...
        if not found_premium_users:
            await aa.edit_text("ɴᴏ ᴀᴄᴛɪᴠᴇ ᴘʀᴇᴍɪᴜᴍ ᴜsᴇʀ ғᴏᴜɴᴅ ɪɴ ᴛʜᴇ ᴅᴀᴛᴀʙᴀsᴇ")
        else:
            try:
                ...
                with open('usersplan.txt', 'w+') as outfile:
                    outfile.write(new)
                await message.reply_document('usersplan.txt', caption="<u>ᴘʀᴇᴍɪᴜᴍ ᴜsᴇʀs</u>:\n\n")
                await aa.delete()
                os.remove('usersplan.txt')
    except Exception as e:
        await aa.edit_text(f"❌ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ: {str(e)}")
...
async def plan(client, message):
    ...
    await message.reply_photo(
        photo="https://iili.io/CJdjRhg.jpg",
        caption=PREMIUM_TXT,
        reply_markup=keyboard)
...
async def restart_bot(b, m):
    ...
    if not is_restarting:
        is_restarting = True
        await m.reply_text("**Hᴇʏ...!! Oᴡɴᴇʀ/Aᴅᴍɪɴ ᴊᴜsᴛ ʀᴇʟᴀx ɪᴀᴍ ʀᴇsᴛᴀʀᴛɪɴɢ...!!**")
        b.stop()
        time.sleep(2)
...
async def tutorial(bot, message):
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("• sᴜᴘᴘᴏʀᴛ •", url="https://t.me/SAND_VILLAGE"), InlineKeyboardButton("•⚡ᴍᴀɪɴ ʜᴜʙ •", url="https://t.me/NARUTO_PUBLIC")]
        ])
    )
# ----------------------------------------
# 𝐌𝐀𝐃𝐄 𝐁𝐘 𝐀𝐁𝐇𝐈
...
async def broadcast_handler(bot: Client, m: Message):
    await bot.send_message(Config.LOG_CHANNEL, f"ʙʀᴏᴀᴅᴄᴀsᴛ sᴛᴀʀᴛᴇᴅ Bʏ {m.from_user.mention}")
...
async def leaderboard_handler(bot: Client, message: Message):
    try:
        ...
        async def generate_leaderboard(filter_type):
            ...
            leaderboard = [f"<b>{filter_title[filter_type]} ᴛᴏᴘ 10 ʀᴇɴᴀᴍᴇʀs</b>\n"]
            ...
            if user_rank:
                leaderboard.append(f"\n<b>ʏᴏᴜʀ ʀᴀɴᴋ:</b> {user_rank} ᴡɪᴛʜ {user_count} ʀᴇɴᴀᴍᴇs")

            leaderboard.append(f"\nʟᴀsᴛ ᴜᴘᴅᴀᴛᴇᴅ: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
            leaderboard.append(f"\n<i>**ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ᴀᴜᴛᴏ-ᴅᴇʟᴇᴛᴇ ɪɴ {Config.LEADERBOARD_DELETE_TIMER} sᴇᴄᴏɴᴅs**</i>")
        ...
        if not leaderboard_text:
            no_data_msg = await message.reply_text("<blockquote>ɴᴏ ʟᴇᴀᴅᴇʀʙᴏᴀʀᴅ ᴅᴀᴛᴀ ᴀᴠᴀɪʟᴀʙʟᴇ ʏᴇᴛ!</blockquote>")
        ...
        error_msg = await message.reply_text(
            "<b>ᴇʀʀᴏʀ ɢᴇɴᴇʀᴀᴛɪɴɢ ʟᴇᴀᴅᴇʀʙᴏᴀʀᴅ!</b>\n"
            f"<code>{str(e)}</code>\n\n"
            f"**ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ sᴇʟғ-ᴅᴇsᴛʀᴜᴄᴛ ɪɴ {Config.LEADERBOARD_DELETE_TIMER} sᴇᴄᴏɴᴅs.**"
        )
        await asyncio.sleep(Config.LEADERBOARD_DELETE_TIMER)
        try:
